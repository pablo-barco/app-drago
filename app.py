    ...
    with tab5:
        ...
        with st.expander("🛠️ Gestionar Tareas y Reparaciones"):
            ...
            for idx, tarea in enumerate(datos["tareas"]):
                ...
                c_t.write(f"• {tarea['nombre']} [{tarea['prioridad']}] - {estado}")
                if c_b.button("🗑️", key=f"del_tar_pan_{idx}"):
        ...
        with st.expander("📅 Gestionar Caducidades de Seguridad"):
            ...
            st.markdown("---")
            st.write("**Caducidades actuales:**")
            for idx, item in enumerate(datos["caducidades_puras"]):
                c_c, c_b = st.columns([4, 1])
                c_c.write(f"• {item['elemento']} (Caduca: {item['fecha_caducidad']})")
                if c_b.button("🗑️", key=f"del_cad_pan_{idx}"):
        ...
        with st.expander("🔧 Gestionar Mantenimientos Críticos"):
            ...
            st.markdown("---")
            st.write("**Mantenimientos actuales:**")
            for idx, maint in enumerate(datos["mantenimientos_mixtos"]):
                c_m, c_b = st.columns([4, 1])
                c_m.write(f"• {maint['elemento']} (Cada {maint['intervalo_horas']}h o {maint['intervalo_meses']} meses)")
                if c_b.button("🗑️", key=f"del_maint_pan_{idx}"):
